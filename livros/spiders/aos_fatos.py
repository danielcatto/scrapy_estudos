...
class AosFatosSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        links = sel.xpath("//section[@id='three-car ds']/a/@href").getall()
    ...
    def parse_noticia(self, response):
        ...
        for quote in quotes:
            ...
            sql = "INSERT INTO news (autor, titulo , quote, data, url, status) VALUES (%s, %s, %s, %s, %s, %s)"
            val = (autor, titulo, quotes_text, data_pub, url, status)
            mycursor.execute(sql, val)
            mydb.commit()
            print(mycursor.rowcount, "record inserted. ", titulo)
            time.sleep(1)
